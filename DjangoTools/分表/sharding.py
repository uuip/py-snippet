...
SHARDING_COUNT_DEFAULT = getattr(settings, "SHARDING_COUNT_DEFAULT", 40)
...
def create_model(abstract_model_class, sharding, meta_options=None):
    ...
    table_name = "%s_%s%s" % (
        abstract_model_class._meta.app_label,
        abstract_model_class._meta.db_table,
        sharding,
    )
...
class ShardingMixin:
    @classmethod
    def shard(cls, sharding_source=None) -> models.Manager:
        sharding = cls.get_sharding(str(sharding_source))
        db_table = f"{cls._meta.app_label}_{cls._meta.db_table}{sharding}"
        if db_table not in shard_tables:
            create_model(cls, sharding)

            cursor = connection.cursor()
            tables = [
                table_info.name for table_info in connection.introspection.get_table_list(cursor)
            ]
    ...
    def get_sharding(cls, sharding_source=None):
        ...
        return str(
            int(sharding_source) % int(getattr(cls, "SHARDING_COUNT", SHARDING_COUNT_DEFAULT))
        )
    ...
    def default_sharding(cls):
        if getattr(cls, "SHARDING_TYPE", "date") == "date":
            date_sharding_format = getattr(
                cls, "SHARDING_DATE_FORMAT", SHARDING_DATE_FORMAT_DEFAULT
            )
